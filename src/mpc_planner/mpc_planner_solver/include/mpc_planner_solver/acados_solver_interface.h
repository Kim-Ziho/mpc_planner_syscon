...
namespace MPCPlanner
{
    struct AcadosParameters
    {
        ...
        void printParameters(YAML::Node &parameter_map)
        {
            ...
            for (int k = 0; k < SOLVER_N; k++)
            {
                ...
                {
                    ROS_INFO_STREAM(it->first.as<std::string>() << ": " << all_parameters[k * SOLVER_NP + it->second.as<int>()]);
                }
            }
        }
    ...
    public:
        struct AcadosInfo
        {
            ...
            {
                ROS_INFO_STREAM("=============== " << "Solver Info" << " ===============");
                ROS_INFO_STREAM("SQP iterations: " << sqp_iter);
                ROS_INFO_STREAM("Minimum time for solve [ms]: " << min_time * 1000);
                ROS_INFO_STREAM("KKT: " << kkt_norm_inf);
                ROS_INFO_STREAM("Solve Time [ms]: " << solvetime * 1000.);
                ROS_INFO_STREAM("NLP Residuals: " << nlp_res);
