...
namespace MPCPlanner
{
    ...
    Solver::~Solver()
    {
        ...
        if (status)
        {
            ROS_INFO_STREAM("Solver_acados_free() returned status " << status);
        }
        // free solver capsule
        status = Solver_acados_free_capsule(_acados_ocp_capsule);
        if (status)
        {
            ROS_INFO_STREAM("Solver_acados_free_capsule() returned status " << status);
        }
    }
    ...
    int Solver::solve()
    {
        ...
        for (int iteration = 0; iteration < _num_iterations; iteration++)
        {
            ...
            double elapsed_this_iter = std::chrono::duration<double>(iter_end - iter_start).count();

            iteration_time_sum += elapsed_this_iter;
            double avg_iteration_time = iteration_time_sum / static_cast<double>(iteration + 1);

            auto now = std::chrono::steady_clock::now();
            double total_elapsed = std::chrono::duration<double>(now - timeout_start).count();

            // Stop iterating if we ran out of time
            if (total_elapsed + avg_iteration_time >= _params.solver_timeout)
            {
                ROS_WARN_STREAM_THROTTLE(15000., "Timeout is enabled. Stopping after " << iteration + 1 << " iterations because planning time is exceeded");
                break;
            }
        }
    ...
    int Solver::completeOneIteration()
    {
        ...
        if (_exit_code_one_iter == ACADOS_SUCCESS)
        {
            if (CONFIG["debug_output"].as<bool>())
                ROS_INFO_STREAM("Solver_acados_solve(): SUCCESS!");
        }
        else
        {
    ...
    {
        if (CONFIG["debug_output"].as<bool>())
            ROS_INFO_STREAM("Initialize Plan with a Braking Plan");
    ...
    void Solver::printIfBoundLimited() const
    {
        ...
        for (int k = 0; k < N; k++)
        {
            for (YAML::const_iterator it = _model_map.begin(); it != _model_map.end(); ++it)
            {
                ...
                {
                    ROS_WARN_STREAM_THROTTLE(500, it->first.as<std::string>() + " limited by lower bound");
                }
                if (std::abs(getOutput(k, it->first.as<std::string>()) - it->second[3].as<double>()) < 1e-2)
                {
                    ROS_WARN_STREAM_THROTTLE(500, it->first.as<std::string>() + " limited by upper bound");
                }
            }
        }
    }
