...
namespace MPCPlanner
{
  ...
  {
    ROS_INFO_STREAM("Decomp Constraints");
    ...
    ROS_INFO_STREAM("DecompConstraints::initialize done");
  }
  ...
  void DecompConstraints::update(State &state, const RealTimeData &data, ModuleData &module_data)
  {
    ...
    if (CONFIG["debug_output"].as<bool>())
      ROS_INFO_STREAM("DecompConstraints::update");
    ...
    if (max_decomp_constraints > _max_constraints)
      ROS_WARN_STREAM("Maximum number of decomp util constraints exceeds specification: " << max_decomp_constraints << " > " << _max_constraints);

    if (CONFIG["debug_output"].as<bool>())
      ROS_INFO_STREAM("DecompConstraints::update done");
  }

  bool DecompConstraints::getOccupiedGridCells(const RealTimeData &data)
  {
    if (CONFIG["debug_output"].as<bool>())
      ROS_INFO_STREAM("GetOccupiedGridCells");
  ...
  void DecompConstraints::setParameters(const RealTimeData &data, const ModuleData &module_data, int k)
  {
    ...
    {
      if (CONFIG["debug_output"].as<bool>())
        ROS_INFO_STREAM("DecompConstraints::setParameters()");
      for (int d = 0; d < _n_discs; d++)
      {
  ...
  void DecompConstraints::visualize(const RealTimeData &data, const ModuleData &module_data)
  {
    ...
    if (CONFIG["debug_output"].as<bool>())
      ROS_INFO_STREAM("DecompConstraints::visualize done");
  }
